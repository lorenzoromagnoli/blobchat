...
function onLocalTracks( tracks ) {
  ...
  for ( let i = 0; i < localTracks.length; i++ ) {
    ...
    if ( localTracks[ i ].getType() === 'video' ) {
      ...
      threeScene.initLocalBubble()
      $( `#localVideo${i}` ).on( 'loadeddata', function() {
        predictor.initTFjs( `localVideo${i}` )
      } )
...
function onConferenceJoined() {
  ...
  isJoined = true;
  threeScene.assignIdToLocalTrack( room.myUserId() );
...
function onConnectionSuccess() {
  ...
  room.addCommandListener( "shapeChanged", function( values ) {
    ...
    threeScene.changeShape( values.attributes.userdID, values.attributes.shapeID );
  } )
}
...
function createConnection( name, scene, pred ) {
